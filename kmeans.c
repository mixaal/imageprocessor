...
int *kmeans_clusters(layer_t layer, rect_t zone, size_t n, color_info_t *result) 
{
  ...
  for(int i=0; i<n; i++) {
      ...
      printf("choose[%d]=%ld\n", i, choose);
      int idx = choose * color_components;
      int cx = choose % layer.width;
      int cy = choose / layer.width;
      choose += step;
      ...
      result[i].intensity = to_gray(result[i].color);
      result[i].variance = vec3_init(0.0f, 0.0f, 0.0f);
      result[i].center= vec3_init((float)cx/zx, (float)cy/zy, 0.0f);
   }
   _Bool swapping = True;
   while(swapping) {
     swapping = False;
...
//#pragma omp parallel for
    for(int y=zone.miny; y<zone.maxy; y++)  {
      for(int x=zone.minx; x<zone.maxx; x++) {
         ...
         int category = -1;
         for(int j=0; j<n; j++) {
           double d = vec5_dist2_double(vec5_init((float)x/zx, (float)y/zy, v), vec5_init(result[j].center.x, result[j].center.y, result[j].color));
           if (d < max_dist) {
             max_dist = d;
             category = j;
           }
